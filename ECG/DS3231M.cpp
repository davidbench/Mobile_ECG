...
bool DS3231M_Class::begin(TwoWire &wirePort, const uint32_t i2cSpeed) {
  /*!
  ...
  */
  ...
  _i2cPort->beginTransmission(DS3231M_ADDRESS);     // Address the DS3231M
  uint8_t errorCode = _i2cPort->endTransmission();  // See if there's a device present
...
uint8_t DS3231M_Class::readByte(const uint8_t addr) {
  ...
  */
  _i2cPort->beginTransmission(DS3231M_ADDRESS);        // Address the I2C device
  _i2cPort->write(addr);                               // Send the register address to read
  _TransmissionStatus = _i2cPort->endTransmission();   // Close transmission
  _i2cPort->requestFrom(DS3231M_ADDRESS, (uint8_t)1);  // Request 1 byte of data
  return _i2cPort->read();                             // read it and return it
}  // of method readByte()
void DS3231M_Class::writeByte(const uint8_t addr, const uint8_t data) {
  ...
  */
  _i2cPort->beginTransmission(DS3231M_ADDRESS);       // Address the I2C device
  _i2cPort->write(addr);                              // Send the register address to write
  _i2cPort->write(data);                              // Send the data to write
  _TransmissionStatus = _i2cPort->endTransmission();  // Close transmission
}  // of method writeByte()
...
DateTime DS3231M_Class::now() {
  ...
  */
  _i2cPort->beginTransmission(DS3231M_ADDRESS);        // Address the I2C device
  _i2cPort->write(DS3231M_RTCSEC);                     // Start at specified register
  _TransmissionStatus = _i2cPort->endTransmission();   // Close transmission
  _i2cPort->requestFrom(DS3231M_ADDRESS, (uint8_t)7);  // Request 7 bytes of data
  if (_i2cPort->available() == 7)                      // Wait until the data is ready
  {
    _ss = bcd2int(_i2cPort->read() & 0x7F);         // Mask high bit in seconds
    _mm = bcd2int(_i2cPort->read() & 0x7F);         // Mask high bit in minutes
    _hh = bcd2int(_i2cPort->read() & 0x3F);         // Mask 2 high bits in hours and clamp to 0-23
    _i2cPort->read();                               // Read and ignore Day-Of-Week register
    _d = bcd2int(_i2cPort->read() & 0x3F);          // Mask 2 high bits for day of month
    _m = bcd2int(_i2cPort->read() & 0x1F);          // Mask 3 high bits for Month
    _y = bcd2int(_i2cPort->read()) + 2000;          // Add 2000 to internal year
